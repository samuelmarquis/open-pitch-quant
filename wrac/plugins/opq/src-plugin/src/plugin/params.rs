...
/// Plain value → display string, shared with the host-facing conversion so
/// the plugin GUI and host generic UI always show identical text.
pub(crate) fn parameter_value_text(id: u32, value: f64) -> PluginResult<String> {
    Ok(value_to_text(param_spec(id)?, value))
}

pub(crate) fn parameter_default_value(id: u32) -> PluginResult<f64> {
    Ok(param_spec(id)?.info.default_value)
}

pub(crate) fn parameter_text_value(id: u32, text: &str) -> PluginResult<f64> {
    text_to_plain(param_spec(id)?, text)
}

/// Host domain == plain domain for every opq parameter (see PARAM_SPECS),
/// so the host edit notification carries the plain value unchanged.
pub(crate) fn parameter_host_value(id: u32, value: f32) -> PluginResult<f64> {
    param_spec(id)?;
    Ok(f64::from(value))
}

/// Pushes every parameter's current value to the GUI. Called by the GUI
/// refresh timer; automation and preset loads reach the WebView this way.
pub(crate) fn notify_gui_parameters(
    shared: &crate::state::SharedState,
    mut notify: impl FnMut(u32, f32),
) {
...
/// Full parameter table as JSON. The GUI builds its controls from this
/// manifest so Rust remains the single source of truth for ranges, defaults,
/// steps, and choice labels.
pub(crate) fn parameter_manifest_json() -> serde_json::Value {
    let params: Vec<serde_json::Value> = PARAM_SPECS
        .iter()
        .map(|spec| {
            let (kind, choices, unit) = match spec.format {
                Format::Percent => ("percent", None, "%"),
                Format::Seconds => ("seconds", None, "ms"),
                Format::Hertz => ("hertz", None, "Hz"),
                Format::Cents => ("cents", None, "ct"),
                Format::Integer => ("integer", None, ""),
                Format::Choice(names) => ("choice", Some(names), ""),
            };
            serde_json::json!({
                "id": spec.info.id,
                "name": spec.info.name,
                "min": spec.info.min_value,
                "max": spec.info.max_value,
                "default": spec.info.default_value,
                "stepped": spec.info.flags.is_stepped,
                "kind": kind,
                "unit": unit,
                "choices": choices,
            })
